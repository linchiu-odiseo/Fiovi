# tutor-aula-navigation — Specification

## Purpose

Define la navegación jerárquica del tutor: **Aula → Curso → Examen**. Expone dos rutas nuevas (`/tutor/aulas/:classroomId` y `/tutor/aulas/:classroomId/curso/:course`) con view-models que agrupan y filtran `TutorExamsStore` en cliente. Warm path: cero requests HTTP cuando el store ya está poblado por el home tutor. Cold path (deep-link): un solo `GetTutorExamsUseCase.execute()` para poblar el store, luego derivaciones reactivas. Depende de `tutor-exam-list` (store compartido) y `tutor-exams-api` (schema con `classroomId` + `course` snapshots).
